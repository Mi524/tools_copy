...
def write2wb(xlsxwriter_wb,highlight_column_list,keyword_dict,keyword_format_dict,highlight_column='Text'):
    ...
    for row_index,value in enumerate(highlight_column_list):  #[7743:7745]
        ...
        #1.写入第一个文档的部分
        if type(params) == str:
            success = xlsxwriter_ws.write(row_index+1,0,params)
        elif type(params) == list and params:  #是列表格式并且不是空列表 
            success = xlsxwriter_ws.write_rich_string(row_index+1,0,*params)
            #如果返回是-5代表列表只有两个元素，不能写入,需要把文本取出来重新写
            if success == -5:
                one_word_text = params[-1]
                success = xlsxwriter_ws.write(row_index+1,0,one_word_text,params[0])
        else:
            success = xlsxwriter_ws.write(row_index+1,0,params)
...
def highlight(xlsxwriter_wb,xlrd_ws,highlight_column,keyword_dict,keyword_format_dict):
    """
    可以自己选处理哪个内容
    """
    #获取columns
    ...
    xlsxwriter_wb = write2wb(xlsxwriter_wb,highlight_column_series,keyword_dict,keyword_format_dict,highlight_column)
...
def highlight_kw(path,keyword_dict,keyword_format_dict):
    ...
    xlsxwriter_wb = highlight(xlsxwriter_wb,xlrd_ws,highlight_column,keyword_dict,keyword_format_dict)
...
if __name__ == '__main__' :
    ...
    highlight_kw('.\\',keyword_dict,keyword_format_dict)
