...
from common_utils.excel_functions import write_format_columns, refresh_excel_calculations1,\
    refresh_excel_calculations2, save_csv

#

def dataframe_write_table(df, table_name, conn):
    try:
...
def check_mapping_complete(df_worksheet, complete_header_df, original2cn_dict, file_tag):
    ...
    check_required_columns = set(
        c_required_columns) - set(df_worksheet.columns)
    ...
    if dtype_dict:
        ...
        for dtype_column, dtype_type in dtype_dict.items():
            ...
            if dtype_column not in list(complete_header_df.columns):
                enter_exit(
                    f'Cannot find "{dtype_column}" in concatnated_data!')
            if dtype_type_temp == '' or dtype_type_temp == '默认' or dtype_type_temp == 'default':
                ...
                complete_header_df = normalize_dates(
                ...
                if output and dtype_type_temp == 'str' and np.issubdtype(complete_header_df[dtype_column].dtype, np.datetime64):
                    complete_header_df[dtype_column] = complete_header_df[dtype_column].dt.strftime(
                        '%Y/%m/%d')
                else:
...
def get_filter_condition_standardize_tag(filter_condition):
    ...
    if is_df_column(filter_condition):
        ...
        filter_condition_list = [x.strip()
                                 for x in filter_condition.split('==')]
...
def process_sort_order(join_table_df, join_columns, string):
    if string != '':
        ...
        reverse_order = [
            True if x.lower() == 'desc' else False for x in reverse_order]
        ...
        reverse_ordere_new = [True for x in range(
            len(join_columns))] + reverse_ordere_new
...
def calc_similarity(not_standard_str, new_standard_str, calc_func):
    ...
    try:  # 也门 -- 对应被拆开的 卡塔尔 也门 科威特 阿曼,第一个卡塔尔拆出来之后 分母为0
        match_list_len_no_space_divide = len(
            not_standard_str_no_space) / len(new_standard_str_no_space)
    except ZeroDivisionError:
        match_list_len_no_space_divide = 0

    str_similarity = match_list_len + continues_match + \
        match_list_len_no_space + match_list_len_no_space_divide
...
def check_similarity_number(not_standard_str, standard_dict, special_syn_list,
    ...
    for standard_str, target_str in standard_dict.items():
        ...
        match_list = number_similarity(
            not_standard_str, new_standard_str, first_syn)
...
def check_similarity_simple(not_standard_str, standard_dict, special_syn_list,
    ...
    for standard_str, target_str in standard_dict.items():
       ...
       # 确认至少两个字符相同，并且如果出现数字，从左往右遇到第一个完整连续的数字串，必须完全相同
        check_number_part = compare_number_part(
            not_standard_str, new_standard_str)
        ...
        str_similarity = calc_similarity(
            not_standard_str, new_standard_str, lcs)
...
def check_similarity_strict(not_standard_str, standard_dict, special_syn_list,
    ...
    for standard_str, target_str in standard_dict.items():
        ...
        str_similarity = calc_similarity(
            not_standard_str, new_standard_str, filter_lcs)

        if str_similarity >= 2:
            # 有3个和3个以上的字符，前3个全是字母确保3个字母都相同，前3个字符有数字和英文，确保英文和数字都相同
            # 前两/三位字母相同,方便后面elif写条件
            first2_letter_equal = not_standard_str[:2] == new_standard_str[:3].split(' ')[
                0][:2]
            first3_letter_equal = not_standard_str[:3] == new_standard_str[:3].split(' ')[
                0][:3]

            # 前3位字符串包含2个数字的情况 必须满足前3个字母相同
            first3_letter_2num_not_standard = re.search(
                '[0-9]{2,}', not_standard_str[:3]) != None
            first3_letter_2num_standard = re.search(
                '[0-9]{2,}', new_standard_str[:3]) != None
...
def get_min_max_date(result_df):
    ...
    if not date_time_df.empty:
        ...
        try:
            min_datetime = datetime.datetime.strftime(
                datetime_series.min(), '%Y.%m.%d')
            max_datetime = datetime.datetime.strftime(
                datetime_series.max(), '%Y.%m.%d')
...
def refresh_configs(config_table_path_list):
    ...
    for c in config_table_path_list:
        print(f'Refreshing excel:{Path(c).name}')
        try:
            refresh_excel_calculations1(c)
        except:
            refresh_excel_calculations2(c)
...
def get_partial_not_match(complete_header_df_notna, seq, source_column,
    ...
    column_order = ['Standization No.', 'Source_column', 'Standard_column',
                    'Content needs to standardized', 'Filter condition', 'Filter content']

    stand_column = '--'.join([standard_table_name, standard_column])

    # 获取模糊匹配(标准化)失败的部分
    find_lack_columns(complete_header_df_notna, [
                      target_column_edit, source_column])
    ...
    partial_match_not_match_df = func_loc(
        partial_match_not_match_df, column_order)
...
def replace_value_func(complete_header_df_new, replace_value_str, input_column, output_column):
    if replace_value_str != '':
        ...
        for replace_value in replace_values:
            ...
            # 支持regex替换
            if replace_original != '' and 'regex(' != replace_original[:6].lower():
                complete_header_df_new[output_column] = complete_header_df_new[output_column].astype(
                    str)
...
def get_replace_dict(string):
    try:
        replace_dict = [x.strip()
                        for x in string.split('\n') if x.strip() != '']
        replace_dict = dict([[y.strip() for y in split_colon(x)]
...
def get_save_name(output_dir, config_file_dir, min_max_date_range):
    ...
    config_name = Path(config_file_dir).name
    save_name = re.split('[_\-]', config_name, maxsplit=1)[-1]
...
def get_save_sheet_name(config_table_name):
    ...
    sheet_name = re.sub(pattern='(.*)(\.xlsx|\.csv|.xls)',
                        repl='\g<1>', string=config_table_name)
...
def process_join_table(join_table_df, join_columns, target_columns, filter_condition, sort_order, join_table_name=''):
    ...
    if filter_condition != '':
        join_table_df = get_filter_condition_match(
            join_table_df, filter_condition)
