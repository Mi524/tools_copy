...
class ConfigReader(object):
    ...
    def __init__(self, config_file_dir, config_table_name, config_list, *args, **kwargs):
        ...
        self.require_file_dir = kwargs.get(
    ...
    def get_complete_header_df(self, header_table_df):
        ...
        standard_column = header_table_df[header_table_columns[0]].fillna(
            '').tolist()
    ...
    def get_config_tables(self, if_walk_path=True):
        ...
        df_workbook = pd.ExcelFile(header_table_path, engine='openpyxl')

        sheet_name_list = df_workbook.book.sheetnames

        table_dict = {}
        for sheet in sheet_name_list:
            sheet_visibility = df_workbook.book.get_sheet_by_name(
                sheet).sheet_state

            if sheet_visibility == 'visible':  # 只读取可见的sheet
                for config in self.config_list:
                    if config in sheet.lower().strip():
                        ...
                        if 'mapping' in config:
                            ...
                            complete_header_df, target_cn_columns = self.get_complete_header_df(
                                table)
                        else:
...
if __name__ == '__main__':
    ...
    table_reader = ConfigReader(
        config_file_dir='.\\', config_list=config_list, config_table_name='config',)
